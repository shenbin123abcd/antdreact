;app.detail=(function( url , settings ,method ){
                      ...
                      <div class='modal-body-content'>
                        ...
                        <form id="detail-order-form">   
                            <div class="content-form-block">
                                <input type="text" placeholder="阁下称呼" class="form-control input-style " id="koubeic_name" value="${hb.store.get("koubeic_name")||''}" />
                                <input type="text" placeholder="阁下手机号" class="form-control input-style  last" id="koubeic_contact"  value="${hb.store.get("koubeic_contact")||''}" />
                                <div class="desc-text">为了能给你提供更好的服务，请留下你的联系方式</div>
                            </div>
                            <div class='modal-dialog-footer f-15'>
                                <div class="modal-dialog-send" >
                                    <div class="line"></div>
                                    <button type="submit" class="submit">去支付</button>
    ...
    `);
        ...
        $('#myOrderModal').on('submit',function (e) {
            e.preventDefault();
            hb.store.set('koubeic_name',$('#koubeic_name').val());
            if(hb.validation.checkPhone($('#koubeic_contact').val())){
                hb.store.set('koubeic_contact',$('#koubeic_contact').val());
            }

            hb.lib.iui.loading.show();
            app.service.submitOrder({
                item_id:hb.location.url('?item_id'),
                shop_id:hb.location.url('?shop_id')||'',
                merchant_pid:hb.location.url('?merchant_pid')||'',
                name:$('#koubeic_name').val(),
                contact:$('#koubeic_contact').val()
            }).then(res=>{
                hb.lib.iui.loading.hide();
                hb.lib.iui.alert('success');
            },res=>{
                hb.lib.iui.loading.hide();
                hb.lib.iui.alert(res);
            })
